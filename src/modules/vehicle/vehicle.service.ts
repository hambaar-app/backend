...
export class VehicleService {
  ...
  async create(
    ...
    return this.prisma.$transaction(async (tx) => {
      ...
      const verificationStatus = await tx.verificationStatus.create({
        data: {},
  ...
  async getAllVehicles(userId: string) {
    ...
    const vehiclesWithUrls = await Promise.all(
      vehicles.map(async (vehicle) => {
        if (
          vehicle.verificationDocuments &&
          typeof vehicle.verificationDocuments === 'object'
        ) {
          const documents = plainToInstance(
            VehicleDocumentsDto,
            vehicle.verificationDocuments,
          ...
          if (
            documents.vehiclePicsKey &&
            Array.isArray(documents.vehiclePicsKey)
          ) {
            presignedUrls.vehiclePics = await Promise.all(
              documents.vehiclePicsKey.map(async (s3Key, index) => {
                try {
  ...
  async update(
    ...
    const oldDocs =
      plainToInstance(VehicleDocumentsDto, vehicle.verificationDocuments) ?? {};
    const newDocs = instanceToPlain(
