...
export class PackageService {
  ...
  async getAllRecipients(
    ...
    return this.prisma.packageRecipient
      .findMany({
        where: {
          address: {
            userId,
  ...
  async getById(id: string, tx: PrismaTransaction = this.prisma) {
    const packageData = await tx.package
      .findFirstOrThrow({
        where: {
          id,
          deletedAt: null,
  ...
  async getAll(
    ...
    const packages = await this.prisma.package
      .findMany({
        where: {
          senderId: userId,
          status: {
  ...
  async updateRequest(requestId: string, session: SessionData) {
    return this.prisma
      .$transaction(async (tx) => {
        const { package: packageData, ...request } =
        ...
        const matchedTrip = session.packages
          .find((p) => p.id === request.packageId)
          ?.matchResults.find((m) => m.tripId === request.tripId);
